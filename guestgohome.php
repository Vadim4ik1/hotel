...
<div class="mx-auto h1" style="width: 200px;">
  ...
  <table class="table table-bordered" >
    ...
    $date=date("Y-m-d");
// echo($date);
    $staff = mysqli_query($connect, "SELECT * FROM `nomera_bron` WHERE `status`='Заселено'");
  ...
  //     endif;
    ...
    <tr>
      ...
      <!-- <td class="<?=$color?>"><?= $st[10] ?></td> -->
         ...
         <!-- <form action="bron/zaselenie.php" method="post" enctype="multipart/form-data"> -->
      ...
      <td>
        ...
        <!--     <li><a href="bron/delete_bron.php?id=<?=$st[0]?>" class="dropdown-item">Удалить</a></li>
            <li><a href="bron/submit.php?id=<?= $st[0]?>" class="dropdown-item" value="">Изменить</a></li> -->

            <li><a href=""class="dropdown-item" data-bs-toggle="modal" value="Выселить" data-bs-target="#exampleModal<?= $st[0]?>">Выселить</a></li>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Черный список</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
         <form action="checktoguest.php" method="post" enctype="multipart/form-data">
       <div class="form-group">
              <!-- <input type="text" name="login" class="form-control px-3 py-3" placeholder="Кратко опишите причину"> -->
              <label for=""> Гость : <?= $st[4] ?></label><br>
               <!-- <label for=""> Статус : <?= $st[12] ?></label><br> -->
             <!-- <input type="hidden" name="id_guest"  value="<?= $st[0]?>"> -->

            </div>    

  <div class="form-group">
                      <label for=""> Добавить к стоимости: 
              <input type="text" name="money_dop"  class="form-control px-3 py-3" value="0">
            </div>
              <input type="hidden" name="money" value="<?= $st[11] ?>">
            <label>ИТОГО : <?= $st[11] ?></label>
              <div class="form-group">

  </div>
        <input type="hidden" name="pass_num" class="form-control px-3 py-3" value="<?= $st[13]?>">
        <input type="hidden" name="number" class="form-control px-3 py-3" value="<?= $st[15]?>">
        <input type="hidden" name="place" class="form-control px-3 py-3" value="<?= $st[14]?>">
        <input type="hidden" name="email" class="form-control px-3 py-3" value="<?= $st[16]?>">

        <input type="hidden" name="id_n" class="form-control px-3 py-3" value="<?= $st[0]?>">

        <input type="hidden" name="money" class="form-control px-3 py-3" value="<?= $st[11]?>">

        <input type="hidden" name="full_name" class="form-control px-3 py-3" value="<?= $st[4]?>">

          <input type="hidden" name="date_in" class="form-control px-3 py-3" value="<?= $st[2]?>">

          <input type="hidden" name="transfer" class="form-control px-3 py-3" value="<?= $st[8]?>">

          <input type="hidden" name="nomer" class="form-control px-3 py-3" value="<?= $st[1]?>">
        <input type="hidden" name="id_n" class="form-control px-3 py-3" value="<?= $st[0]?>">
        <input type="hidden" name="food" class="form-control px-3 py-3" value="<?= $st[7]?>">
      </div>
